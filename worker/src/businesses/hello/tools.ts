...
const formFieldSchema = z.object({
  name: z.string().describe("stable field key, for example city or budget"),
...
export const helloTools: ToolDef[] = [
  ...
  },
  {
    id: "create_artifact",
    description:
      "Create a durable workspace artifact that appears in the right-side Artifacts panel. Use this when the user asks to generate an HTML page, report, markdown document, JSON artifact, or local preview.",
    inputSchema: z.object({
  ...
  },
  {
    id: "ask_user_form",
    description:
      "Render an inline form in the chat and wait for the user's structured input. Use this when free-form chat would be too ambiguous and you need specific fields before continuing.",
    inputSchema: z.object({
